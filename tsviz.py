...
from hyperspectral.common_fun import *
...
PYTHON_FILE_PATH = os.path.join("hyperspectral", "stored_recovered")
LOGDIR = os.path.join(PYTHON_FILE_PATH, "logs")
SPECTRAL_DATA_PATH = os.path.join("hyperspectral","SOCS.mat")
OBSERVER_DATA_PATH = os.path.join("hyperspectral","CIEobs.mat")
ILLA_PATH = os.path.join("hyperspectral","IllA.mat")
ILLD65_PATH = os.path.join("hyperspectral","IllD65.mat")
N_SAMPLES = 0
N_TRAIN = 0
N_VAL = 0
N_TEST = 0
TRAIN_LOG_DIR = os.path.join("hyperspectral","logs")
TRAIN_SPLIT = 0.8
VAL_SPLIT = 0.15
...
def CSALoss(y_true, y_pred):
    normalize_r = tf.math.l2_normalize(tf.transpose(y_pred),axis=0)
    normalize_t = tf.math.l2_normalize(tf.transpose(y_true),axis=0)
...
class VisualizationToolbox:
    ...
    def loadDatasets(self):
        ...
        if DATASET == Dataset.HYPERSPECTRAL:
            #dataPath = "./datamark-internettrafficdata.csv"
            #modelName = 'test_ae_mlp_recovered_{}_csa'.format(LATENT_DIM)
            modelName = "keras_model_csa"
            self.modelPath = os.path.join("hyperspectral", 'models', modelName, "model.h5")

            # name for the standard model
            self.standardModelPath = self.modelPath
            self.standardModelName = modelName
            ...
            rec_ref_D65_data = np.vstack(load_data(os.path.join("hyperspectral", "stored_recovered", "rec_sd_D65.dat")))
            ...
            self.trainX, self.trainY = train_set, train_set
            self.testX, self.testY = test_set, test_set
            self.valX, self.valY = val_set, val_set
        ...
        if DATASET != Dataset.INTERNET_TRAFFIC and DATASET != Dataset.HYPERSPECTRAL:
    ...
    def loadModel(self):
        ...
        #net = autoencoder.mlp_1D_network( os.path.join(self.modelPath,'config.json'))
        #net.add_model(addr=os.path.join(self.modelPath, 'best_epoch'), modelName='csa_best')
        # Load the model
        """f = h5py.File(self.modelPath,'r+')
        data_p = f.attrs['training_config']
        decoded = data_p.decode().replace("learning_rate","lr").replace("\"name\": \"Adam\", ", "")
        decoded = decoded.replace("\"\"CSALoss\"\"", "\"CSALoss\"")
        data_p = decoded.encode()
        f.attrs['training_config'] = data_p
        f.close()"""
        self.model = tf.keras.models.load_model(self.modelPath, custom_objects={ 'CSALoss': CSALoss })
        #self.model.name = "Model"
        #self.model = net
        #self.model.name = self.standardModelName
        self.currentlyLoadedModel = self.standardModelPath

        print("Model summary:")
        self.model.summary()

        #out = self.model.evaluate(self.trainX, self.trainY, verbose=True)
        ...
        elif DATASET == Dataset.HYPERSPECTRAL:
            self.labelsPlaceholder = tf.placeholder(tf.float32, shape=[None, 31])
            self.loss = CSALoss(self.labelsPlaceholder, requiredGradients[-1])
        else:
            self.labelsPlaceholder = tf.placeholder(tf.float32, shape=[None, 1])
            ...
            if (prevLayerName != currentLayerRootName):
                ...
                if (layerIdx < len(self.outputGradientsWrtLayer)):
                    gradientsWrtInput, gradientsWrtOutput, gradientsWrtLoss, tensorSlice, loss = self.sess.run([self.layerGradientsWrtInput[idx], \
                            ...
                            feed_dict={self.inputPlaceholder: np.expand_dims(currentInput, axis=0), self.labelsPlaceholder: np.expand_dims(np.array(currentLabel), axis=0)})
    ...
    def performInverseOptimization(self, y, iterations=1000, stepSize=1.0, randomStart=True, optimizeOnlyRawValues=True,
        ...
        if randomStart:
            ...
            #optimizedInput = np.random.normal(loc=0.0, scale=0.25, size=(1, self.trainX.shape[1]))
            if smoothSampling:
