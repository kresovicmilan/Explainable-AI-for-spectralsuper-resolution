...
def createPlotImg(filterName, filterImportance, filterCluster, maxFilterCluster, rawValues, saliency, plotForecast=False,
    ...
    pointStride = plotWidth / (rawValues.shape[0])
    ...
    if plotAxisLabels:
        ...
        # Horizontal Scale
        text ="000"
        step = (filterWidth - padding - (len(text) * int(plotDim / 400)) - (padding - (len(text) * int(plotDim / 400))))/rawValues.shape[0]
        idx = 0
        for w in range(400, 710, 10):
            text = str(w)
            if w < 540 or w > 570:
                cv2.putText(dataPlot, text, (int(padding - (len(text) * int(plotDim / 400) - idx*step)), filterHeight - 30 * int(plotDim / 1000)), cv2.FONT_HERSHEY_SIMPLEX, 0.4 * int(plotDim / 1000), (0.0, 0.0, 0.0, 1.0), 2, cv2.LINE_AA)
            idx += 1
    ...
    def drawPoint(i, channelIdx=None):
        ...
        if showSaliency:
            ...
            if saliency.size == 1:
                ...
                clr = saliency[i]
            cv2.circle(dataPlot, currentPoint, 25, (1.0, 1.0 - clr, 1.0 - clr, 1.0), -1)
...
def plotInputData(useSaliency, saliencyMap=None):
    inputData = np.expand_dims(np.array(data[inputLayerIdx]), axis=0)
    ...
    for idx, filterName in enumerate(inputFeatureNames):
        ...
        if saliencyMap is None:
            saliency = np.expand_dims(np.array(data[-1][0][saliencyMapIdx]), axis=0)[idx, :]
        else:
            saliency = np.expand_dims(saliencyMap, axis=0)[idx, :]
...
def plotLayer(layerID, useSaliency, useImportance, useCluster, numFilters=3, plotInputSaliency=False, createCombinedLayerPlot=False):
    ...
    for layerID in layers:
        ...
        for filterIdx in topFilterIdx:
            ...
            dataPlot = createPlotImg(filterName=filterName, filterImportance=filterImportance, filterCluster=None, maxFilterCluster=None, rawValues=rawValues,
            ...
            for currentTuple in [(False, False, False, False, False), (False, True, False, False, False), (False, True, True, False, False), (True, True, True, True, False), (True, True, True, True, True)]:
                ...
                for filterIdx in filterList:
                    ...
                    dataPlot = createPlotImg(filterName=filterName, filterImportance=filterImportance,
                                             filterCluster=None, maxFilterCluster=None,
                ...
                combinedPlot = createCombinedPlot(allFilters, gridView=gridView, filtersInfo=None, maxPlotsPerRow=-1)
...
if __name__ == "__main__":
    ...
    url = "http://172.17.0.2:6005/viz/api/"
    ...
    while True:
        ...
        if plotExample:
            if batchMode:
                ...
                outputDirectory = os.path.join(rootDirectory, rootDirectory + str(exampleIdx))
